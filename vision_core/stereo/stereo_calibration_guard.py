...
"""Fail-closed runtime guard for conditionally active stereo calibrations."""
...
class StereoCalibrationGuard:
    ...
    def _validate_activation_record(self) -> dict[str, Any]:
        ...
        if not isinstance(envelope, dict) or set(envelope) != REQUIRED_CHANNELS:
            raise CalibrationGuardError(
                "INVALID_ACTIVATION_RECORD", "temperature envelope is invalid"
            )
        ...
        for channel in sorted(REQUIRED_CHANNELS):
            ...
            if not limits["minimum_c"] <= value <= limits["maximum_c"]:
                violations.append(
                    ...
                    f"[{limits['minimum_c']:.4f}, {limits['maximum_c']:.4f}]C"
                )
        if violations:
            raise CalibrationGuardError(
