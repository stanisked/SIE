...
def _score_threshold(value):
 ...
 return float(value)
def artifact(score_threshold: float = .5) -> DetectorArtifact:
 threshold=_score_threshold(score_threshold)
 return DetectorArtifact(ARTIFACT_SCHEMA_VERSION,"mp-persondet-opencv","1","opencv_mp_persondet","2023mar",MODEL_SHA256,OUTPUT_CONTRACT_VERSION,"person",CONFIDENCE_SEMANTICS,threshold,{"input_size":224,"nms_threshold":.3,"score_threshold":threshold,"backend":"opencv_cpu","public_bbox_semantics":"FULL_BODY_ROI_FROM_MEDIAPIPE_VIRTUAL_KEYPOINTS_V1"})
...
class MPPersonDetOpenCV:
 def __init__(self, model:Path, reference:Path, score_threshold: float = .5):
 ...
 def detect(self, frame_bgr:np.ndarray):
  ...
  keep=cv2.dnn.NMSBoxes(xy.tolist(),scores.tolist(),self.score_threshold,.3,top_k=5000); out=[]
  for i in np.asarray(keep).reshape(-1):
   x1,y1,x2,y2=xy[int(i)]; score=float(scores[int(i)])
   if math.isfinite(score) and self.score_threshold<=score<=1 and x2>x1 and y2>y1:
